...
def test_register(username, password, email, open_page):
    ...
    driver = open_page
    # 点击立即注册
    ele_click(driver, (By.XPATH, '//a[@href|text()="立即注册"]'))
    # 输入注册信息
    ele_send_key(driver, (By.XPATH, '//label[contains(text(),"用户名")]/following-sibling::*[1]'), username)
    ele_send_key(driver, (By.XPATH, '//label[contains(text(),"密码")]/following-sibling::*[1]'), password)
    ele_send_key(driver, (By.XPATH, '//label[contains(text(),"确认密码")]/following-sibling::*[1]'), password)
    ele_send_key(driver, (By.XPATH, '//label[contains(text(),"邮箱")]/following-sibling::*[1]'), email)
    # 点击注册按钮
    ele_click(driver, (By.XPATH, '//*[@class="reg_sub"]/*[@value="注 册"]'))

    # 断言1：判断数据库表df_user_userinfo内是否存在username
    result = sm.execute([f"SELECT uname FROM df_user_userinfo WHERE uname='{username}'"])
