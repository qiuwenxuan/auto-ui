...
class SQLManager(object):
    parseConf = ParseConf()
    sql_delete = [
        sql.strip() for sql in parseConf.get_value("SQL", "sql_delete").split(",")
    ]
    ...
    def __init__(self, default_file=SQL_FILE):
        ...
        logger.debug(f"连接数据库资源，数据库文件:{default_file}")
    ...
    def setup_sql(self):
        logger.debug("正在初始化测试数据...")
        self.execute(self.sql_delete)
        self.execute(self.sql_insert)
        logger.debug(f"初始化测试数据完成，初始化数据为:{self.execute(self.sql_select)}")
